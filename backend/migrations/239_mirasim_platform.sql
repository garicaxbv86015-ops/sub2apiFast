-- Add Mirasim as a first-class platform (relay device-ticket & OpenAI/Claude compatible protocol).
--
...
ALTER TABLE user_platform_quotas
    ...
    CHECK (platform IN ('anthropic', 'openai', 'gemini', 'antigravity', 'grok',
                        'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'mirasim'));
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('mirasim' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'mirasim'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('mirasim' IN template_constraint_def) = 0 THEN
