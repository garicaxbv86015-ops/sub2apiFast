...
// 验证在允许不安全 HTTP 时，API Key 账号可使用自定义 HTTP Base URL。
func TestBuildGrokResponsesRequestAllowsHTTPBaseURLWhenConfigured(t *testing.T) {
	account := &Account{
		Platform: PlatformGrok,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
	...
	req, err := buildGrokResponsesRequest(context.Background(), nil, account, []byte(`{"model":"grok-4.3"}`), "api-key", "", cfg)
	require.NoError(t, err)
