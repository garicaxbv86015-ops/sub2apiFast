export default {
    accounts: {
      ...
      cnProviders: {
        ...
        window5hTooltip: '5 小时滚动窗口用量（所有模型共享）',
        windowWeeklyTooltip: '7 天滚动窗口用量（所有模型共享）',
        windowMonthlyTooltip: '月度窗口用量',
        window7dClaudeTooltip: 'Claude 系列模型 7 天滚动额度用量',
        window7dFableTooltip: 'Fable 模型 7 天滚动额度用量（同时计入 Claude 额度）',
        probe: '查询',
      ...
      mirasim: {
        apiKeyLabel: 'Mirasim Token / 登录凭据',
        apiKeyPlaceholder: 'ey...（Mirasim JWT Token 或中继凭据）',
        apiKeyHint: '请输入 Mirasim 登录 Token（JWT）或中继凭据。可从本地 ~/.mirasim/setting.json 的 auth.token 提取，也可通过 Auth 授权生成。',
        deviceId: '设备 ID（Device ID）',
        deviceIdPlaceholder: '可选，留空将自动从私钥公钥推导',
        privateKey: '设备私钥（Private Key）',
        privateKeyPlaceholder: '可选，用于自动签署设备票据（Hex 或 Base64 编码的 Ed25519 私钥）',
        deviceHint: '若提供设备私钥与设备 ID，Sub2API 将自动换发并维护短期设备票据与签名头。',
        protocolRules: {
          title: '模型协议分流',
          hint: '自适应模式下按模型匹配上游协议。支持精确 ID 或末尾 * 通配（如 claude-*）；自上而下第一条命中生效；未命中走 Chat Completions。',
          patternPlaceholder: 'claude-* 或 gpt-6-astra',
          add: '添加规则',
          remove: '删除规则',
          restoreDefaults: '恢复默认',
          fallback: '未命中以上规则 → Chat Completions（/v1/chat/completions）',
      ...
      types: {
        ...
        mirasimOauth: 'Mirasim 授权登录',
        mirasimOauthDesc: '浏览器 OAuth、邮箱验证码或本机导入，自动生成设备身份',
        mirasimManual: '手动录入凭据',
        mirasimManualDesc: '手动填写 Token、设备私钥与设备 ID',
      ...
      oauth: {
        ...
        mirasim: {
          title: 'Mirasim 授权登录',
          followSteps: '请按照以下步骤完成 Mirasim 授权：',
          step1GenerateUrl: '生成授权链接',
          providerLabel: '第三方登录源',
          providerGithub: 'GitHub 账号登录',
          providerGoogle: 'Google 账号登录',
          generateAuthUrl: '生成授权链接',
          step2OpenUrl: '在浏览器中完成授权',
          openUrlDesc: '点击下方按钮在浏览器中打开 Mirasim 授权页面，登录并确认授权：',
          openAuthUrl: '打开 Mirasim 授权页面',
          step3EnterCode: '输入授权码或重定向 URL',
          authCodeDesc: '授权完成后，将浏览器地址栏中的完整 URL 或 code 参数粘贴到下方：',
          authCode: '授权码 / 回调 URL',
          authCodePlaceholder: '粘贴完整的回调 URL (http://localhost:port/callback?code=...) 或授权码',
          authCodeHint: 'Sub2API 将自动换取 Token 并派生 Ed25519 设备公私钥与 Device ID',
          importantNotice: '提示：授权成功后系统将自动生成设备密钥并换取短期会话票据，无需手动配置私钥',
          emailCodeAuth: '邮箱验证码登录',
          localAppAuth: '从本机 Mirasim.app 导入',
          emailCodeDesc: '输入注册 Mirasim 的邮箱，获取验证码一键登录并自动生成设备身份：',
          emailInputLabel: 'Mirasim 邮箱地址',
          emailPlaceholder: '请输入登录邮箱',
          sendCode: '发送验证码',
          resendCode: '重新发送',
          codeInputLabel: '邮箱验证码',
          codePlaceholder: '6 位数字验证码',
          codeHint: '请输入邮件中收到的验证码',
          verifyAndLogin: '验证并登录',
          verifying: '正在验证...',
          localAppTitle: '本机 Mirasim 客户端凭据一键导入',
          localAppDesc: '自动从本机 ~/.mirasim 目录读取当前登录的 Token 与设备私钥',
          localAppPathsHint: '将读取以下配置文件：',
          importLocalBtn: '一键导入本机凭据',
          importing: '正在导入...',
          refreshTokenAuth: '手动输入 RT',
          refreshTokenDesc: '使用已有 Mirasim Refresh Token 换取新票据并自动生成设备身份：',
          refreshTokenLabel: 'Mirasim Refresh Token',
          refreshTokenPlaceholder: '输入 Mirasim Refresh Token',
          refreshTokenHint: '系统将验证 Refresh Token 并生成设备身份',
          validateAndCreate: '验证并创建账号',
          validating: '正在验证...',
          pleaseEnterRefreshToken: '请输入 Refresh Token',
          failedToValidateRT: '验证 Refresh Token 失败',
          failedToGenerateUrl: '生成 Mirasim 授权链接失败',
          missingExchangeParams: '缺少 code / session_id / state',
          failedToExchangeCode: 'Mirasim 授权码兑换失败',
          failedToImportLocal: '导入本机 Mirasim 凭据失败',
          failedToSendCode: '发送验证码失败',
          failedToVerifyCode: '验证码校验失败',
          devCodeNotice: '本地开发模式验证码：{code}',
