...
describe('UserPlatformQuotaModal', () => {
  ...
  it('渲染包含 Mirasim 的全部 11 个平台，初始限额为空', async () => {
    const w = await mountAndOpen()
  ...
  it.each(['kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'mirasim'] as const)(
    '保存 %s 的修改时保留其他平台的限额', async (platform) => {
      const existing: PlatformQuotaUpdateItem[] = [
        { platform: 'openai', daily_limit_usd: 10, weekly_limit_usd: 20, monthly_limit_usd: 100 },
        ...(['kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'mirasim'] as const).map(p => ({
  ...
  it('保存提交完整 11 平台配额', async () => {
    ...
    expect(uid).toBe(99)
    expect(payload).toHaveLength(11) // 始终提交全部 11 个平台
